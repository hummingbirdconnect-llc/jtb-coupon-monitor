...
- 「海外旅行」「国内旅行」タブを順に開く
- 画面上で表示中のカードだけを対象にする
- 各タブ内の「詳細を見る」アコーディオンをすべて先に開く
- 海外・国内のタブごとに1回だけ画面撮影する（通常は合計2回）
- 撮影画像を画面上のカード座標で切り抜き、クーポン別の証拠画像として保存する
...
GitHub Actionsは公式API・埋め込みJSON・HTML・Playwrightの順で取得し、ページ内容のhashが変わった場合に `codex_audit_queue/` へ監査候補JSONを保存します。OpenAI APIは実行しません。HISは公式画面の確認に成功した場合、検証済みフィードをGitHubへ保存します。WordPressへの同期は、国外IP制限を避けるため国内Macの定期処理が行います。記事本文、下書き、公開状態は変更しません。
...
WordPress認証情報はGitHub Actionsでは使いません。国内Macの `vault_box/tools/.env` だけから読み、Gitへ保存しません。

### HIS一覧のWordPress自動同期

- GitHub ActionsはHIS公式画面の取得、件数・時刻・コード照合、フィード保存までを担当
- 国内Macは `vault_box/tools/sync-his-wordpress-feed-local.sh` で最新フィードを取得して送信
- `com.hbconnect.his-wordpress-sync` が各GitHub実行後に2回確認し、同じ確認時刻は再送しない
- 国内Macの `tools/.env` に `YF_WP_URL`、`YF_WP_USER`、`YF_WP_APP_PASSWORD` が必要
...
- ログは `/Users/takamacmini/logs/his-wordpress-sync/` に保存
